...
const GameCard = ({
  ...
  const handleAutoImport = async (e) => {
    e?.stopPropagation();
    if (!appid || downloading) return;
    setDownloading(true);
    try {
      const res = await invoke('auto_save_and_import_lua', { appid, gameName: currentTitle });
  ...
  const handleCheckManifest = async (e) => {
    ...
    try {
      ...
      if (res.ready) {
        ...
        // Lua exists but missing manifest/key -> considered not ready
        setManifestStatus('not_found');
        // Short hint for user
      ...
      // Legacy fallback if backend lacks check_install_ready
      try {
