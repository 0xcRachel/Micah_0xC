...
const CHARACTER_IMAGE_PATH = charImg;
    ...
    <div className="relative w-full h-full flex items-end justify-center pointer-events-none">
      ...
      >
        ...
        <button
          ...
          className="absolute w-14 h-14 rounded-full flex items-center justify-center pointer-events-auto
            bg-[#30302e]/90 text-[#e6dfd3] shadow-2xl border border-[#e6dfd3]/10
            backdrop-blur-sm cursor-pointer opacity-0
            hover:bg-[var(--led-color)] hover:text-white transition-colors duration-200"
          style={{ scale: 0 }}
      ...
      >
        {(displayImage || CHARACTER_IMAGE_PATH) ? (
          ...
          <div
            onClick={toggleOpen}
            className="mb-8 p-4 border-2 border-dashed border-[var(--card-border)]/30 rounded-xl
              bg-[var(--card-bg)]/60 backdrop-blur-sm text-center cursor-pointer pointer-events-auto"
          >
            <span className="text-sm font-semibold text-[var(--text-muted)]">Character Image Placeholder</span>
          </div>
        )}
      </div>
