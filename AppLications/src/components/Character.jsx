...
const Character = ({ 
  ...
  const toggleOpen = contextSafe(() => {
    if (!isOpen) {
      ...
      // 2. Spheres fly out one by one with stagger after character settles
      ...
      // 1. Spheres spring back with a bounce
      tl.to(sphere3Ref.current, {
        scale: 0,
        opacity: 0,
        x: 0,
        y: 0,
  ...
  return (
    <div className="relative w-full h-full flex items-end justify-center pointer-events-none">
      ...
      {/* Floating Action Spheres (z-index BEHIND character = z-[2]) */}
      <div 
        ref={containerRef}
      ...
      {/* Character Image (z-[3] = on top of the spheres) */}
      <div 
        className="relative flex items-end justify-center h-full w-full pointer-events-auto"
        style={{ zIndex: 3 }}
      >
        {(characterImage || CHARACTER_IMAGE_PATH) ? (
          <img
            ...
            alt="Character"
            onClick={toggleOpen}
            className="h-[110%] object-contain object-bottom cursor-pointer"
